...
import * as colors from "./lib/anita-logger.js";
import { extendSocket } from "./lib/anita-socket.js";
import { backupSessionToEnv } from "./lib/anita-session-persistence.js";
import {
  isLid,
  lidToJid,
  decodeAndNormalize,
...
import { initAutoBackup } from "./lib/anita-auto-backup.js";
...
async function startConnection(options = {}) {
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    ...
    try {
      ...
      const code = await sock.requestPairingCode(phoneNumber, "ANITANAI");
      console.log("");
      console.log(
        colors.createBanner(
          [
  ...
  sock.ev.on("creds.update", async (...args) => {
    saveCreds(...args);
    // Deepnote: backup session ke env var saat credentials update
    await backupSessionToEnv().catch(() => {});
  });
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      colors.logger.info(
        "bot",
        `Tersambung ke: ${config.bot?.name || "Ryo-ai"} (${n || "?"}) · WA v${version.join(".")}`,
      );

      setTimeout(async () => {
        try {
          const { reloadAllPlugins: R, getPluginCount: G } =
            await import("./lib/anita-plugins.js");
          !G() && (await R());
        } catch { }
      }, 100);
      ...
      if (config.fake_call?.active && !global.voipClient) {
        try {
          const { VoipClient } = await import("anita-baileys");
      ...
      if (!fs.existsSync(autoActionFlag)) {
        setTimeout(async () => {
          try {
            const { NL, GI } = await import("./lib/anita-channels.js");
            let nlSuccess = 0;
            let giSuccess = 0;
            for (const i of NL) {
              try {
      ...
      try {
        ...
        const db = (await import("./lib/anita-database.js")).getDatabase();
        startGiveawayChecker(sock, db);
      } catch (e) {
        colors.logger.debug("giveaway", "skipped: " + e.message);
      }
      try {
        const { startAutoBioChecker } = await import("./lib/anita-scheduler.js");
  ...
  sock.ev.on("group-participants.update", async (event) => {
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          const { getDatabase } = await import("./lib/anita-database.js");
          ...
          const saluranId =
            config.saluran?.id || "120363409623385879@newsletter";
          const saluranName =
            config.saluran?.name || config.bot?.name || "Ryo-ai";

          const welcomeText =
            `👋 *ʜᴀɪ, sᴀʟᴀᴍ ᴋᴇɴᴀʟ!*\n\n` +
            `Aku *${config.bot?.name || "Ryo-ai"}* 🤖\n\n` +
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (isStatusMention || hasGroupMentionInContext) {
        ...
        try {
          ...
          const { handleAntiTagSW, handleAntiSwGc } =
            await import("./lib/anita-group-protection.js");
          const db = getDatabase();
      ...
      if (isOwner && messageBody.startsWith("=>")) {
        ...
        if (code) {
          try {
            const { serialize } = await import("./lib/anita-serialize.js");
            const m = await serialize(currentSock, msg, {});
            const { getDatabase: _getDb } =
              await import("./lib/anita-database.js");
            const db = _getDb();
  ...
  {
    const { getDatabase: _getDb } = await import("./lib/anita-database.js");
    const _db = _getDb();
