...
import fsc from "fs";
import { getDatabase } from "./anita-database.js";
import { saluranCtx } from "./anita-context.js";
import { getAssetBuffer } from "./anita-asset-manager.js";
...
function createContextInfo(jid, text, title = "", body = "", thumbnail = null) {
  ...
  if (jid && text) {
    ...
    contextInfo.stanzaId = "ANITAAI" + Date.now();
  }
...
async function serialize(sock, msg, store = {}) {
  ...
  m.botName = sock.user?.name || config.bot?.name || "Ryo-ai";
  m.messageId = m.id;
  m.chatId = m.chat;
  m.senderId = m.sender;
    ...
    if (replyVariant === 2) {
      ...
      return sock.sendMessage(
        await ensureResolved(m.chat),
        {
          ...
          jpegThumbnail: srtImage ? await sharp(srtImage).resize(300, 300).toBuffer() : await sharp(getAssetBuffer("anita2"))
            .resize(300, 300)
            .toBuffer(),
          caption: text,
          ...defaultOptions,
      ...
      const sss = getAssetBuffer("anita3");
      return sock.sendMessage(
        await ensureResolved(m.chat),
        {
          video: getAssetBuffer("anita-mp4"),
          caption: text,
          gifPlayback: true,
          contextInfo: {
            ...contextInfo,
      ...
      const thumbnailBuf = srtImage || getAssetBuffer("anita-baileys");
      const fakeOrder = {
        key: {
          participant: "0@s.whatsapp.net",
          remoteJid: m.chat,
        ...
        message: {
          orderMessage: {
            ...
            status: 1,
            surface: 1,
            message: config.bot?.name || "Ryo-ai",
      ...
      const thumbnailBuf = randomImg || srtImage || await getAssetBuffer("anita-baileys");
