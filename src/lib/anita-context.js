...
const keys = [
  ["anita-games", (buf) => { gameThumbBuffer = buf; }],
  ["anita-rpg", (buf) => { rpgThumbBuffer = buf; }],
  ["anita-winner", (buf) => { winnerThumbBuffer = buf; }],
];
...
function getRpgContextInfo(title, body) {
  ...
  if (title || body) {
    base.externalAdReply = {
      title: title || config.bot?.name || "Anita RPG",
      body: body || "",
...
function createFakeQuoted(botName = "Ryo-ai", verified = true) {
  return {
    key: {
      fromMe: false,
