...
import { addJadibotOwner } from "./anita-jadibot-database.js";
import { extendSocket } from "./anita-socket.js";
  ...
  if (usePairing && !state.creds?.registered) {
    try {
      ...
      if (m && m.chat) {
        let thumbnail = null;
        try {
          if (!!getAssetBuffer("anita2")) {
            thumbnail = getAssetBuffer("anita2");
          }
        } catch { }
