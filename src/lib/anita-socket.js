...
import { downloadMediaMessage, getContentType } from "anita-baileys";
import { addExifToWebp, DEFAULT_METADATA } from "./anita-exif.js";
...
async function extendSocket(sock) {
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "Ryo-ai";
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "Ryo-ai";
    if (source) {
      let data = source;
