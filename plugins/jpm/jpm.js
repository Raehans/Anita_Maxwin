...
import { fetchGroupsSafe } from "../../src/lib/anita-jpm-helper.js";
import {
  getAutoJpmConfig,
  setAutoJpmConfig,
...
} from "../../src/lib/anita-auto-jpm.js";
import { getMimeType, getExtension } from "../../src/lib/anita-utils.js";
import * as timeHelper from "../../src/lib/anita-time.js";
import {
  getBinaryNodeChild,
...
try {
  cachedThumb = getAssetBuffer("anita2");
} catch {}
...
async function startAutoJpmSession(m, sock, db) {
  ...
  return sendInteractiveMessage(m, sock, {
    title: `🔄 ${config.bot?.name || "Ryo-ai"} AutoJPM`,
    body,
    footer: `${config.bot?.name || "Ryo-ai"} AutoJPM`,
    buttons,
  });
}
...
async function handleSetDelay(m, sock, db, input) {
  ...
  if (!input) {
    ...
    return sendInteractiveMessage(m, sock, {
      title: `⏱️ ${config.bot?.name || "Ryo-ai"} Delay`,
      body,
      footer: `${config.bot?.name || "Ryo-ai"} JPM System`,
      buttons,
    });
  }
