import { getCaseCount, getCasesByCategory } from "../../case/anita.js";
import {
...
import {
  formatUptime,
  getTimeGreeting,
} from "../../src/lib/anita-formatter.js";
import {
  getCommandsByCategory,
  getCategories,
} from "../../src/lib/anita-plugins.js";
import { getDatabase } from "../../src/lib/anita-database.js";
import fs from "fs";
...
async function formatTime(date) {
  const timeHelper = await import("../../src/lib/anita-time.js");
...
async function buildMenuText(
  ...
  txt += `    ᯓ INFO BOT\n`;
  txt += `╭  • Name : ${botConfig.bot?.name || "Ryo-ai"}\n`;
  txt += `┆  • Author : ${botConfig.bot?.developer || "Owner"}\n`;
  txt += `┆  • Versi : ${botConfig.bot?.version || "1.2.0"}\n`;
  txt += `┆  • Type script : Case x ${botConfig.bot?.developer || "Owner"}\n`;
  txt += `┆  • Uptime : ${uptimeFormatted}\n`;
  txt += `╰➤------------------------------\n`;
  
  txt += `    ᯓ INFO DEVELOPER\n`;
  txt += `╭➤- • DEV RYO YAMADA MD\n`;
  txt += `┆  • Anita Putri Azzahra\n`;
  txt += `╰➤------------------------------\n`;
...
function getContextInfo(
  ...
  const saluranName =
    botConfig.saluran?.name || botConfig.bot?.name || "FebxyZ YT</>";
  const saluranLink = botConfig.saluran?.link || "https://whatsapp.com/channel/0029Vb8pJjxB4hdRpWZeJg0Z";
  const ctx = {
    mentionedJid: [m.sender],
    forwardingScore: 9,
    isForwarded: true,
    externalAdReply: {
      title: botConfig.bot?.name || "Ryo-ai",
...
async function handler(m, { sock, config: botConfig, db, uptime }) {
  ...
  try {
    imageBuffer = fs.readFileSync(botConfig.assets["anita-baileys"])
    thumbBuffer = fs.readFileSync(botConfig.assets["anita2"])
    videoBuffer = fs.readFileSync(botConfig.assets["anita-mp4"])
  } catch (e) {
  ...
  const saluranId = botConfig.saluran?.id || "120363412350560864@newsletter";
  const saluranName =
    botConfig.saluran?.name || botConfig.bot?.name || "Ryo-ai";
  const saluranLink =
  ...
  try {
    ...
    switch (menuVariant) {
      case 1:
        if (videoBuffer) {
          // V1 tetap mempertahankan tampilan/menu yang sama, tetapi media
          // mengikuti cara V5-LV2: prepareWAMessageMedia + gifPlayback.
          const mediaV1 = await prepareWAMessageMedia({
            video: videoBuffer,
            gifPlayback: true,
...
`,
                  ...
                  footer: {
                    text: config.bot?.name || "Ryo Yamada MD",
                  },
                  contextInfo: {
                    mentionedJid: [m.sender],
                    isForwarded: true,
          ...
          await sock.relayMessage(m.chat, msgV1.message, {
            messageId: msgV1.key.id,
          });
        } else {
          await m.reply(text);
        }
      ...
      case 2:
        ...
        const media = await prepareWAMessageMedia({
          image: fs.readFileSync(config.assets["anita-baileys"])
        }, { upload: sock.waUploadToServer })
...
> 👤 *Owner*: ${config.bot?.developer}
> 🧩 *Library*: \`anita-baileys\`
> 👨‍💻 *Developer*: \`Anita Putri\`
                  ...
                  nativeFlowMessage: {
                    messageParamsJson: JSON.stringify({
                      ...
                      tap_target_configuration: {
                        ...
                        canonical_url: "https://anita.site",
      ...
      case 3:
        const content = {
          buttonsMessage: {
            ...
            locationMessage: {
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["anita-baileys"])).resize(300, 170).toBuffer(),
              name: config.bot.name,
...
> 🍬 *Register*: ${user.isRegistered ? "Sudah" : "Belum"}`,
      ...
      case 4: {
        const thumbnail = await sharp(fs.readFileSync(config.assets["anita-baileys"])).resize(300, 300).toBuffer()
        const qvideo = {
          key: {
            fromMe: false,
            participant: m.sender
        ...
        const media4 = await prepareWAMessageMedia({
          video: fs.readFileSync(config.assets["anita-mp4"]),
          gifPlayback: true
        }, { upload: sock.waUploadToServer });
...
`
        ...
        case7Text += `││  Owner : Anita Putri Azzahra\n`;
        ...
        const { getAssetBuffer } = await import("../../src/lib/anita-asset-manager.js");
        const imageBuffer = await getAssetBuffer("anita2");
        const sharp = (await import("sharp")).default;
        const stickerBuf = await sharp(imageBuffer).resize(512, 512).webp().toBuffer();

        const { prepareWAMessageMedia } = await import("anita-baileys");
        ...
        const { generateWAMessageFromContent } = await import("anita-baileys");
        const menuMedia = await prepareWAMessageMedia({
          image: await getAssetBuffer("anita-baileys")
        }, { upload: sock.waUploadToServer });

        const videoLive = await prepareWAMessageMedia({
          video: await getAssetBuffer("anita-mp4")
        }, { upload: sock.waUploadToServer });
      ...
      case 8: {
        ...
        const { getAssetBuffer } = await import("../../src/lib/anita-asset-manager.js");
        const imageBuffer = await getAssetBuffer("anita-baileys");
        const favB = await getAssetBuffer("anita2");
        const sharp = (await import("sharp")).default;
        ...
        const { prepareWAMessageMedia, generateWAMessageFromContent } = await import("anita-baileys");
    ...
    if (audioEnabled) {
      const audioUrl = botConfig.assets["anita-mp3"];
      try {
        switch (menuVariant) {
          case 1:
            try {
          ...
          case 2: {
            ...
            try {
              ...
              await sock.sendMessage(m.chat, {
                audio: fs.readFileSync(config.assets["anita-mp3"]),
                mimetype: "audio/mpeg",
                ptt: false,
              }, { quoted: qpoll });
            }
