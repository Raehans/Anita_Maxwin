...
const pluginConfig = {
  ...
  usage:
    ".autoai on/off --anitamode=<character|custom> --logic=<custom instruction> --type=<text|voice> --mode=<onlychat|assistant>",
  example: ".autoai on --anitamode=furina --type=voice --mode=onlychat",
  isOwner: false,
  isPremium: false,
  isGroup: true,
  isPrivate: false,
...
` +
  ...
  if (subcmd === "global") {
    ...
    if (globalMode === "on") {
      const modeMatch = fullArgs.match(/--anitamode=(\w+)/i);
      ...
      const logicMatch = fullArgs.match(
        /--logic=(.+?)(?=\s+--(?:anitamode|type|logic|mode)|$)/i,
      );
      ...
      if (charKey === "custom" && customLogic) {
        ...
        if (existingGlobal.instruction) {
          ...
          return m.reply(
            `❌ Belum ada persona global yang diset!\n\n> .autoai global on --anitamode=furina\n> .autoai global on --anitamode=custom --logic=...`,
          );
        }
      } else {
        const charList = [
  ...
  if (!mode || !["on", "off"].includes(mode)) {
    ...
    txt += `*Penggunaan:*\n`;
    txt += `> .autoai on --anitamode=<karakter|custom> --type=<text|voice> --mode=<onlychat|assistant>\n`;
    txt += `> .autoai off\n`;
    txt += `> .autoai tambahpersona nama | logic\n`;
    txt += `> .autoai hapuspersona nama\n`;
    ...
    txt += `*Contoh:*\n`;
    txt += `> .autoai on --anitamode=furina --type=text\n`;
    txt += `> .autoai on --anitamode=custom --logic=kamu adalah nexa ai\n`;
    txt += `> .autoai tambahpersona nexa | kamu adalah nexa ai\n`;
    txt += `> .autoai global on --anitamode=furina`;
    return m.reply(txt);
  }
  ...
  if (!charKey) {
    ...
    return m.reply(
      `❌ Karakter tidak valid!\n\n> Karakter tersedia: ${charList}\n\n> Contoh: .autoai on --anitamode=furina --type=voice\n> Custom: .autoai on --anitamode=custom --logic=kamu adalah nexa ai`,
    );
  }
  ...
  if (charKey === "custom") {
    if (!customLogic) {
      return m.reply(
        `❌ Mode custom membutuhkan --logic!\n\n> Contoh: .autoai on --anitamode=custom --logic=kamu adalah nexa ai, ...`,
      );
    }
