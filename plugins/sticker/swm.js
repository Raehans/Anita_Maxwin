...
import { addExifToWebp, isAnimatedWebp, DEFAULT_METADATA } from '../../src/lib/anita-exif.js'
...
async function handler(m, { sock, config: botConfig }) {
    ...
    if (!quoted) {
        return m.reply(
            ...
            `*ᴄᴏɴᴛᴏʜ:*\n` +
            `> \`${m.prefix}swm Ryo-ai\`\n` +
            `> \`${m.prefix}swm Ryo-ai|LuckyArchz\` _(packname + author)_`
        )
    }
    ...
    if (!input) {
        return m.reply(
            ...
            `*ᴄᴏɴᴛᴏʜ:*\n` +
            `> \`${m.prefix}swm Ryo-ai\`\n` +
            `> \`${m.prefix}swm Ryo-ai|LuckyArchz\` _(+ author)_`
        )
    }
