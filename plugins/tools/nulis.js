...
const fontUrl = getAssetBuffer("anita-font");
...
async function handler(m, { sock }) {
  ...
  const inputUrl = getAssetBuffer("anita-kertas");
  if (!inputUrl) {
    return m.reply(
  ...
  try {
    ...
    if (!_fontRegistered) {
      try {
        const fontBuf = getAssetBuffer("anita-font");
        if (fontBuf) {
    ...
    const bgBuf = getAssetBuffer("anita-kertas");
