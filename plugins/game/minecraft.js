...
import {
  ...
  healPlayer,
  JACKPOT_POOLS,
} from "../../src/lib/anita-minecraft.js";
import {
  biomes,
  travelRequirements,
  pickaxes,
  ...
  SMELT_RECIPES,
  CRAFT_RECIPES,
} from "../../src/lib/anita-minecraft-data.js";
...
let thumbMC = null;
try {
  const p = path.join(process.cwd(), "assets", "images", "anita-minecraft.jpg");
  if (fs.existsSync(p)) thumbMC = fs.readFileSync(p);
} catch (e) {}

function ctx() {
  const sId = config.saluran?.id || "120363412350560864@newsletter";
  const sName = config.saluran?.name || config.bot?.name || "Ryo-ai";
  return {
